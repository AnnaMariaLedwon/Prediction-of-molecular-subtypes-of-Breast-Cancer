...
Now now let's load the Breast Cancer Proteomes data: https://www.kaggle.com/piotrgrabo/breastcancerproteomes 
```{r}
###loading the data
...
The patient IDs are in a different format in the clinical dataset and the proteomic data set. This bit of code reformats the id in the clinical data set to allow joining of the two data sets on this variable.
```{r}
...
Now let's count the missing data by column and plots a graph to show the proportion of missing data for each variable.
```{r}
###missing data
NA_counts <- colSums(is.na(proteomes))/nrow(proteomes)
plot(sort(NA_counts, decreasing = TRUE), col = 'blue', type = 'h', xlab = "index of proteome", 
...
How many patients there are with a given molecular subtypes of breast cancer?
```{r}
###barplot
...
## Lasso Data
### Feature selection 
The Lasso regressions is used to select features introduced into the machine learning models.
...
Now let's move on to the visualization of the first 50 proteins obtained by lasso regression.
```{r}
...
plot_grid(plotlist = plot_list,ncol = 1, nrow = 1) #only first one (in the BC_analysis you'll find the code for all)
...
plot_grid(plotlist = plot_list,ncol = 1, nrow = 1) #only one (in the BC_analysis you'll find the code for all)
```

Using the biomaRt package, let's create a csv file with 50 lasso proteins (gene ID, protein ID, description, times).
...
print(stab_var.biomart)
```

### Creation the TRAIN and TEST data for Lasso data
* 70% for TRAIN data
* 30% for TEST data
...
The three models were created for the Lasso data:
...
### Data modification, train and test dataset creation for PAM50
```{r}
...
train_data_pam50.plot$data <- "train_data_pam50"
...
### Models Building
For the PAM50 data I created the same models like for data obtained by lasso regression.
